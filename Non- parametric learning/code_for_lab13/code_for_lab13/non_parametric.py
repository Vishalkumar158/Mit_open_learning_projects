...
def auto_xval(data, labels, k=10, decision_tree=False, verbose=True, config=None):
    model_eval = dt_eval if decision_tree else nn_eval
...
'''
# print("Cross-Validation and Evaluation for Decision Tree:")
# decision_tree_avg_acc = auto_xval(auto_data, auto_labels, decision_tree=True, verbose=True, config=None)
# print("Average Decision Tree Accuracy:", decision_tree_avg_acc)

# Cross-validate and evaluate nearest-neighbor classifier
print("\nCross-Validation and Evaluation for Nearest-Neighbor Classifier:")
nn_avg_acc = auto_xval(auto_data, auto_labels, decision_tree=False, verbose=True, config=None)
print("Average Nearest-Neighbor Classifier Accuracy:", nn_avg_acc)
######################################################################
...
for k in range(1, 21):  # Try k from 1 to 20
    print(f"\nEvaluating Nearest-Neighbor Classifier with k = {k}:")
    nn_avg_acc = auto_xval(auto_data, auto_labels, decision_tree=False, verbose=True, config=k)

    if nn_avg_acc > best_acc:
        best_acc = nn_avg_acc
        best_k = k
