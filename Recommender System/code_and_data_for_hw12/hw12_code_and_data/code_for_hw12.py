...
prediction_with_offset=np.dot(u_a.T,np.array([[1],[10]]))+b_u_a+3.0  #for this specific problem value of b_v=1 and for scond part its 5 and for third its 3
print('prediction with offset',prediction_with_offset)
...
prediction_without_offset=np.dot(u_a_no_b.T,np.array([[1],[10]]))
print('prediction without offset',prediction_without_offset)
...
def load_model():
    with open("ALSmodel", "rb") as file:
        return pickle.load(file)

# Function to find the favorite genre of a user
def find_favorite_genre(user_id, model, genres_dict):
    u, _, v, _ = model  # Assuming u and v are from the loaded model

    # Get the user's ratings from the factorized matrices
    user_ratings = [(i, r) for (a, i, r) in data if a == user_id]

    # Identify movies rated 5.0 by the user
    favorite_movies = [(i, r) for (i, r) in user_ratings if r == 5.0]

    # Find genres of the favorite movies
    favorite_genres = []
    for movie_id, _ in favorite_movies:
        if movie_id in genres_dict:
            favorite_genres.extend(genres_dict[movie_id])

    # Count the occurrences of each genre and find the most common one
    genre_counts = {genre: favorite_genres.count(genre) for genre in genres}
    favorite_genre = max(genre_counts, key=genre_counts.get)

    return favorite_genre

# Assuming you have loaded the model and genres properly
model = load_model()  # Load the saved model using load_model()

# Call the function to find the favorite genre of the synthetic user
synthetic_user_id = 270894
favorite_genre = find_favorite_genre(synthetic_user_id, model, genres_dict)
print("Favorite genre of user", synthetic_user_id, "is:", favorite_genre)

#number of movies in this list match their favorite genre
movies_dict, genres_dict = load_movies()
model = load_model()
(u, b_u, v, b_v) = model
...
sim = {}
for i in movies_dict:
    sim[i] = v[2628].T.dot(v[i])/(np.linalg.norm(v[2628])*np.linalg.norm(v[i]))
...
#movie with highest nad lowest similarity
for genre in genres:
