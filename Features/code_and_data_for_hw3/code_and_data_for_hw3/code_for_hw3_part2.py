...
def xval_learning_alg(learner, data, labels, k):
    n, _ = data.shape  # Update the unpacking of dimensions
    idx = list(range(n))
    np.random.seed(0)
    np.random.shuffle(idx)
    data, labels = data[idx, :], labels[idx, :]  # Update indexing

    s_data = np.array_split(data, k, axis=0)  # Update splitting
    s_labels = np.array_split(labels, k, axis=0)  # Update splitting

    score_sum = 0
    for i in range(k):
        data_train = np.concatenate(s_data[:i] + s_data[i+1:], axis=0)  # Update concatenation
        labels_train = np.concatenate(s_labels[:i] + s_labels[i+1:], axis=0)  # Update concatenation
        data_test = np.array(s_data[i])  # Update indexing
        labels_test = np.array(s_labels[i])  # Update indexing
